      ...
      {content && (
        ...
        <div className='detail-content'>
          <div className='detail-title'>
            <h1>{content.name || content.title} (
              {(
                content.first_air_date || content.release_date || '____'
              ).substring(0,4)}
            )</h1>
          </div>
          ...
          <div className='watch-trailer'>
            <button onClick={() => navigateToExternalUrl(`https://www.youtube.com/watch?v=${video}`)}>Watch Trailer</button>
          </div>
        </div>
      </div>
      )}
